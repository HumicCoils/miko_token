...
import { DynamicExclusionManager } from './modules/DynamicExclusionManager';
...
class Phase4BKeeperBot {
  ...
  private exclusionUpdateInterval?: NodeJS.Timeout;
  ...
  private initializeModules(): void {
    ...
    // Create Anchor provider and vault program
    ...
    // Create vault program instance
    this.vaultIdl.address = this.config.programs.vault_program_id;
    const vaultProgram = new Program(this.vaultIdl, provider);
    ...
    this.dynamicExclusionManager = new DynamicExclusionManager(
      this.connection,
      this.config,
      vaultProgram
    );
    
    // Pass dynamic exclusion manager to distribution engine
    this.distributionEngine.setDynamicExclusionManager(this.dynamicExclusionManager);
