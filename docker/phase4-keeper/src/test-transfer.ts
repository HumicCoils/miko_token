...
async function main() {
  try {
    console.log('🔍 Testing Token Transfer Fee Behavior...\n');
    console.log('Testing deployer (exempt) → test account (non-exempt) transfer\n');

    const connection = new Connection(RPC_URL, 'confirmed');

    // Load Phase 4B config
    const configPath = '../minimal-config.json';
    const config = JSON.parse(fs.readFileSync(configPath, 'utf-8'));
    const mintPubkey = new PublicKey(config.token_mint);
    console.log('✅ Token Mint:', mintPubkey.toBase58());

    // Load deployer keypair (has tokens and is fee-exempt)
    const deployerPath = '../phase4b-deployer.json';
